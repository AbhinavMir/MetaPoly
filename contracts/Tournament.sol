...
contract Tournament is ITournament, ERC721URIStorage, Pausable, AccessControl {using Counters for Counters.Counter;
    ...
    constructor(string memory _baseURI, uint256 _registrationFee, uint256 _startTime, uint256 _TTL, uint256 _maxPlayers, string[] memory _propertyNames, uint256[] memory _propertyPrices, uint256 _maxCells) {_setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        baseURI = _baseURI;
        ...
        for (uint8 i = 0; i < _propertyNames.length; i++) {properties.push(Property(_propertyNames[i], _propertyPrices[i], 0, address(0), 0));}}

    function nextPlayer() internal {_playerMoveCounter.increment();}

    function joinTournament() external {require(_playerIdCounter.current() < maxPlayers, "Tournament is full");
        require(block.timestamp < startTime + TTL, "Tournament has already started");
        require(playerByAddress[msg.sender].playerAddress == address(0), "Player already joined");
        require(token.balanceOf(msg.sender) >= registrationFee, "Not enough tokens to join");
    ...
    function _addPlayer(address _playerAddress) internal {require(playerByAddress[_playerAddress] == false, "Player already exists");
    ...
    function forfeit(address _player) external onlyRole(PLAYER) {require(_player == msg.sender, "Only player can forfeit");
        require(playerByAddress[_player].playerAddress == msg.sender, "Player not found");
        _removePlayer(_player);
        emit playerForfeited(_playerId);}

    function _removePlayer(address _playerAddress) internal {if (hasRole(ACTIVE_PLAYER, _playerAddress)) {_removeRole(ACTIVE_PLAYER, _playerAddress);}
        revokeRole(PLAYER, _playerAddress);
        playerByAddress[_playerAddress]._isActive = false;
        _playerIdCounter.decrement();}

    function movePlayer(uint8 _numberDice, address _playerAddress) external activePlayer(_playerAddress) {playerByAddress[_playerAddress].position = (playerByAddress.position + _numberDice) % MAX_CELLS;
    ...
    function endTurn(address _playerAddress) external activePlayer(_playerAddress) {revokeRole(ACTIVE_TURN, msg.sender);
        grantRole(PLAYER, msg.sender);
        _playerMoveCounter.increment();
        emit turnEnded(_playerMoveCounter.current(), msg.sender);}

    function buildHouses(uint8 _propertyIndex, uint8 _numberOfHouses) external activePlayer(msg.sender) {require(msg.sender == Property[_propertyIndex].owner.playerAddress, "You are not the owner of this property");
        for (uint8 i = 0; i < _numberOfHouses; i++) {_buildHouse(_propertyIndex);}}

    function goToJail(address _player) external onlyRole(BANKER) {grantRole(JAILED_USER, playerByAddress[_player].playerAddress);
    ...
    function payRent(uint8 _propertyIndex) external onlyPlayer {require(Property[_propertyIndex].owner != msg.sender, "You are the owner of this property");
        ...
        if (Property[_propertyIndex].isMortgaged) {_rent = 0;} else if (Property[_propertyIndex].houseCounter > 0) {_rent = _rent + (_houseCounter * _rent);} else {_rent = _rent;}

        _pay(_rent, _houseOwner, msg.sender);}

    function _pay(uint256 _amount, address _to, address _from) internal {require(_amount > 0, "Amount must be greater than 0");
    ...
    function _transferProperty(address _to, uint8 _propertyIndex) internal {require(Property[_propertyIndex].owner == msg.sender, "You are not the owner of this property");
    ...
    function buyProperty(uint8 _propertyIndex, uint8 _playerId, address _player) external activePlayer(_player) {/*
        bytes32 name;
        uint8 price;
        uint8 position;
        uint8 baseRent;
        ...
        */
        ...
        require(playerByAddress[_player].balance >= properties[_propertyIndex].price, "You don't have enough money");
    ...
    function sellProperty(uint8 _propertyIndex, uint8 _playerId) external {require(properties[_propertyIndex].owner.playerAddress == msg.sender, "You are not the owner of this property");
        require(properties[_propertyIndex].isOwned == true, "Property is not owned by anyone");
        require(properties[_propertyIndex].isMortgaged == false, "Property is mortgaged");
    ...
    {uint256 oldNetWorth = playerByAddress[_player].netWorth;
        ...
        for (uint8 i = 0; i < playerByAddress[_player].propertyOwned.length; i++) {newNetWorth += (properties[playerByAddress[_player].propertyOwned[i]].price) / 2;}

        newNetWorth += playerByAddress[_player].balance;
        return newNetWorth;}

    function finePlayer(uint8 _playerId, uint256 amount) internal {require(playerByAddress[_playerId].balance >= amount, "You don't have enough money");
        playerByAddress[_playerId].balance -= amount;
        emit playerPaidIncomeTax(_playerId);}

    function _addAdmin(address _newAdmin) internal {grantRole(DEFAULT_ADMIN_ROLE, _newAdmin);}

    function _buildHouse(uint8 _propertyIndex) internal {require(properties[_propertyIndex].owner.balance > properties[_propertyIndex].houseCost, "Not enough funds");
    ...
    function calcuateRent(uint8 _propertyIndex) internal view returns (uint256)
    {// base rent + house rent * house counter
        return properties[_propertyIndex].baseRent + (properties[_propertyIndex].houseRent * properties[_propertyIndex].houseCounter);}

    function destroyHouse(uint8 _propertyIndex, uint256 numberOfHouses) external activePlayer(properties[_propertyIndex].owner.playerAddress) {require(properties[_propertyIndex].houseCounter >= numberOfHouses, "Not enough houses");
        properties[_propertyIndex].houseCounter -= numberOfHouses;
        // return the owner 50% of the house cost ~ this is reduced house tax
        properties[_propertyIndex].owner.balance += (properties[_propertyIndex].houseCost * numberOfHouses) / 2;}

    function _isActive(address user) internal view returns (bool) {require(Player[user]._isActive == false, "User is not active");
        return true;}

    modifier activePlayer(address _playerAddress) {require(msg.sender == _playerAddress, "Not Player");
